  ...
  readonly parts: Part[]
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (projectId !== "" && collectionId !== undefined && name !== "") {
            setSaving(true)
            try {
                createAssessment(projectId, parseInt(collectionId, 10), name, status, location, date, description).then(async (assessment: Assessment) => {
                    for (var i = 0; i < parts.length; i++) {
                        await createAssessmentPart("Planned", assessment.id, parts[i].id)
                    }
                    setSaving(false)
                    router.refresh()
                    toast({
                    ...
                    <div className="min-w-40 flex flex-col space-y-2">
                        <Select onValueChange={(value) => setCollectionId(value)}>
                            <SelectTrigger className="focus:ring-offset-indigo-400 focus:ring-transparent">
                                <SelectValue 
                                    placeholder={"Select an assessment collection"} 
